// Called from the Sing Ova Sundays page before it lets someone log in.
...
// sos_verifications has no public policies at all.
...
Deno.serve(async (req) => {
  ...
  const emailRes = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      from: FROM,
      to: [email.trim()],
      subject: "Your Sing Ova Sundays code",
      html: `
        <p>Here's your login code for Sing Ova Sundays:</p>
        <p style="font-size:28px;font-weight:700;letter-spacing:4px;">${code}</p>
        <p>Enter it on the page to log in. It expires in ${CODE_TTL_MINUTES} minutes.</p>
