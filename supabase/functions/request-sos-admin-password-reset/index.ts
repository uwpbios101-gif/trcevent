// Called from the Sing Ova Sundays admin "Forgot password?" link. Sends a
// 6-digit code by email (never a clickable link -- see comp intake
// verification for why). Clone of request-admin-password-reset retargeted
// at sos_city_admins instead of comp_admins -- reset-admin-password itself
// is generic (only touches admin_password_resets + Supabase Auth, not any
// specific admin-type table), so it's reused as-is for both rosters.
// Always returns the same generic response regardless of whether the email
// matches a real admin, to avoid confirming who's on the roster.
...
Deno.serve(async (req) => {
  ...
  await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      from: FROM,
      to: [email.trim()],
      subject: "Reset your Sing Ova Sundays admin password",
      html: `
        <p>Here's your password reset code for the Sing Ova Sundays admin page:</p>
